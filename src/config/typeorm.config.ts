...
export class TypeOrmConfigService implements TypeOrmOptionsFactory {
  ...
  createTypeOrmOptions(): TypeOrmModuleOptions {
    ...
    const sqlLogging: LoggerOptions =
      rawLogging === 'false'
        ? false
        : (rawLogging.split(',').map((level) => level.trim()) as LoggerOptions);

    const replicaAccessEnabled = this.configService.get<boolean>(
      'app.enableReplicaDbAccess',
    );
    ...
    if (replicaAccessEnabled && replicaHost && replicaHost !== host) {
      return {
        ...
        extra: {
          ...
          ), // Maximum time (ms) to wait for a new connection before timing out.
          statement_timeout: this.configService.get<number>(
            'app.statementTimeout',
          ), // Terminates queries that exceed the timeout (in ms).
          idle_in_transaction_session_timeout: this.configService.get<number>(
            'app.idleInTransactionSessionTimeout',
          ), // Terminates idle transactions after the specified time (in ms).
          maxUses: this.configService.get<number>(
            'app.maxUsesBeforeRecreatingConnection',
          ), //Recreate connections after 'n' uses
        },
        logging: sqlLogging,
