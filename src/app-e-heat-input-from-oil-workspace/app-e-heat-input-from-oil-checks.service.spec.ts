...
describe('Appendix E Heat Input From Oil Checks Service Test', () => {
    ...
    beforeEach(async () => {
        ...
        appEHeatInputFromOilRepository = module.get<AppEHeatInputFromOilWorkspaceRepository>(
    ...
    describe('Appendix E Heat Input From Oil Checks', () => {
        ...
        it('Should fail APPE-50 check when there is a duplicate', async () => {
            jest.spyOn(appEHeatInputFromOilRepository, 'findDuplicate')
                .mockReturnValue(mockDuplicate);

            let result;
            try {
                result = await service.runChecks(
                    mockDTO,
                    null,
                    appETestRunId,
                );
