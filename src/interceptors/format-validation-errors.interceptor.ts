  ...
  intercept(_context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      catchError(err => {
        ...
        console.log(err);
        if (err.response &&
            err.response.statusCode && err.response.statusCode === 400 &&
            (err.response.message as string[]) && err.response.message.length > 0
        ) {
          err.response.message = err.response.message.map((e: string) => e.replace(regex, ''));
        }
