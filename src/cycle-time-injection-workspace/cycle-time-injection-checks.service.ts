...
import {CycleTimeInjectionBaseDTO, CycleTimeInjectionImportDTO} from "../dto/cycle-time-injection.dto";
...
} from "../cycle-time-summary-workspace/cycle-time-summary-workspace.repository";
...
export class CycleTimeInjectionChecksService {
    ...
    async runChecks(
        ...
        const cycleTimeSummary = await this.cycleTimeSummaryRepo.findOneWithAncestors(cycleTimeSumId);

        error = await this.cycle20Check(cycleTimeInjectionId, dto, cycleTimeSummary);
        if (error) {
    ...
    async cycle20Check(
        ...
        if(dto.gasLevelCode) {
            const duplicate = await this.repo.findDuplicate(cycleTimeInjectionId, testSummary.id,
                dto.gasLevelCode);
