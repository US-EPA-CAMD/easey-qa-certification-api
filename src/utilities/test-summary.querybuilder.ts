...
export const addJoins = (
  ...
  return query.innerJoinAndSelect('ts.location', 'ml')
  ...
  if (beginDate && endDate) {
    query.andWhere(
      new Brackets(qb1 => {
        qb1.where(
          new Brackets(qb2 => {
            qb2.where('ts.reportingPeriod IS NULL')
              .andWhere('ts.beginDate IS NOT NULL')
              .andWhere('ts.beginDate BETWEEN :beginDate AND :endDate', { beginDate, endDate })
              .andWhere('ts.endDate IS NOT NULL')
              .andWhere('ts.endDate BETWEEN :beginDate AND :endDate', { beginDate, endDate })
          })
        ).orWhere(
          new Brackets(qb3 => {
            qb3.where('ts.reportingPeriod IS NOT NULL')
